<template>
    ...
    <el-row :gutter="24">
      ...
      <el-col :span=lan2 v-for="video in videos" :key="video.id">
        <video-player :video="video"/>
                {{ video.name }}<el-button type="primary" @click="editMonitor(video)">编辑</el-button>
  ...
  import { list,deleteByIds } from "@/api/modules/monitor"
...
import { dicPage } from "@/api/modules/dic"
  export default {
    ...
    closeFather(val) {
      ...
      generateFakeVideos(count) {
        // 生成一些假的视频数据，实际应用中应从数据库获取
        const fakeVideos = [];
        for (let i = 0; i < count; i++) {
          fakeVideos.push({
            id: `video_${i}`,
            streamUrl: `http://your_stream_url/${i}`,
            name: `Camera Feed ${i}`,
          });
        }
        return fakeVideos;
